...
- `.agents/`: repo-local dot-agents skills, durable work items, and sync tooling.
...
- Keep small, self-contained work in the current conversation.
- Use `.agents/work/<category>/<work-slug>/` when continuity, coordination, handoff, auditability, durable decisions, or an explicit request justifies repository context.
- Follow `.agents/work/AGENTS.md` for the canonical work-item lifecycle, evidence, and closeout contract.
- Store only reusable cross-work findings in `.agents/research/`.
- Refresh upstream-managed workflow files with `.agents/scripts/sync.sh --force`; this repository tracks `main`, and upstream content wins while project guidance and user work remain preserved.
